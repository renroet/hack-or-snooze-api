...
class User {
  ...
  async addToFavorites(evt) {
    ...
    $click.html('&#9733;')
    const $li = $click.closest("li")
    const $storyID = $li.attr("id")

    const res = await getStoryFromID($storyID) 
    const story = new Story(res)
    this.favorites.unshift(story)
    await this.updateFavorites($storyID, "POST")
    
  }

  async removeFromFavorites(evt) {
    const $click = $(evt.target)
    $click.html('&#9734;')
    const $li = $click.closest("li")
    const $storyID = $li.attr("id")
    ...
    const res = await getStoryFromID($storyID)
    const story = new Story(res)
    const idx = this.favorites.indexOf(story)
    this.favorites.splice(idx, 1)
    await this.updateFavorites($storyID, "DELETE")
  }
